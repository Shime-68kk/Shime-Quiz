...
## Runtime summary
Phase 24E adds a test-only scaffold that exercises StorageAdapter reads, restore preview, dry-run restore, confirmed restore, and post-write verification.
The scaffold is not imported by production runtime files and is not wired into production backup/export/restore UI.
