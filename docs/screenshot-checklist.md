...
Capture the Dashboard first-run callout in an empty-data state. The screenshot should show that the callout points to Library safe start options and does not auto-load or auto-save anything.

### 11. Library empty-state onboarding

Capture the Library empty-state onboarding when there are no saved quiz items. Include the guidance for **Dùng quiz mẫu**, JSON/CSV import, text/Markdown import, manual AI copy/paste workflow, and separate EduGen document import.

### 12. Library demo sample quickstart and preview

Capture **Dùng quiz mẫu** before clicking it, then capture the resulting demo sample preview and advisory quality review before confirm-save. Do not imply the sample is auto-saved, AI-generated by Shime, API-backed, or EduGen-powered.
