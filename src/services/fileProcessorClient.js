...
export function isSupportedPdfFileName(fileName) {
  ...
  return name.endsWith('.pdf') && !name.endsWith('.pdf.zip');
}
...
function normalizeEduGenError(payload, fallbackCode = 'edugen_extract_failed', status = null) {
  ...
  const message = cleanString(payload?.error) || 'Không trích xuất được nội dung PDF.';
...
export function mapEduGenExtractResponse(payload, status = 200) {
  ...
  if (!cleanedText) {
    return createError('edugen_empty_text', 'Không tìm thấy nội dung chữ trong PDF.', { status });
  }
...
export async function extractSingleFile(file, options = {}) {
  if (!file || typeof file !== 'object') {
    return createError('edugen_missing_file', 'Chưa chọn file PDF.');
  }

  if (!isSupportedPdfFileName(file.name)) {
    return createError('edugen_unsupported_file', 'Chỉ hỗ trợ file PDF trong bước này.');
  }
  ...
  try {
    ...
    try {
      ...
      return createError(
        'edugen_invalid_json',
        response.ok ? 'EduGen trả về JSON không hợp lệ.' : 'Không trích xuất được nội dung PDF.',
