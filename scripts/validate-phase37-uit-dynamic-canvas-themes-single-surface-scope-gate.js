...
const REVIEW_FILE = 'docs/review/phase37-uit-dynamic-canvas-themes-single-surface-scope-gate.md';
const SUMMARY_FILE = 'docs/release/phase37-uit-dynamic-canvas-themes-single-surface-scope-gate-summary.md';
const SEED_FILE = 'docs/planning/phase37-uiu-dynamic-canvas-themes-single-surface-preview-pilot-seed.md';
const VALIDATOR_FILE = 'scripts/validate-phase37-uit-dynamic-canvas-themes-single-surface-scope-gate.js';
...
const DECISION_TOKEN = 'PHASE37UIT_DYNAMIC_CANVAS_THEMES_SINGLE_SURFACE_SCOPE_GATE_DECISION';
const EXPECTED_DECISION = 'PASS_TO_PHASE37UIU_DYNAMIC_CANVAS_THEMES_SINGLE_SURFACE_PREVIEW_PILOT';
...
const SELECTED_SURFACE_CONSTRAINTS = [
  'future pilot must be non-persistent',
  'scoped to one Dashboard preview surface only',
  'must not become a theme picker',
  'must not write localStorage or sessionStorage',
  'must not mutate global app theme',
  'must not change body/html or app root theme',
  'must not change route behavior or page rendering',
  'must not touch Study Room scoring/answer state',
  'must not touch storage/import/parser/scheduler/data',
  'contrast',
  'focus-visible',
  'reduced-motion',
  '375px',
...
const CANDIDATE_ROWS = [
  'Dynamic Canvas Themes Single-Surface Preview Pilot',
...
const GUARDRAILS = [
  ...
  'runtime implementation in Phase 37-uiT',
  'Dynamic Canvas Themes runtime in Phase 37-uiT',
  'full Dynamic Canvas Themes runtime',
  'full theme picker runtime',
...
const SEED_REQUIRED_TEXT = [
  'runtime preview pilot only if scoped to the selected Dashboard Dynamic Canvas token preview surface',
  'non-persistent',
  'must not implement a theme picker',
  'write localStorage/sessionStorage',
  'account-synced preferences',
  'global app theme or body/html theme',
  'CSS variable theme engine',
  'scoped CSS tokens/classes',
  'routing',
  'handlers',
...
function assertDecisionToken(text) {
  ...
  for (const match of matches) {
    if (!ALLOWED_DECISIONS.includes(match[1])) fail(`Unsupported Phase 37-uiT decision token: ${match[1]}`);
  }
  if (!matches.some(match => match[1] === EXPECTED_DECISION)) {
    fail(`Phase 37-uiT must select expected decision: ${EXPECTED_DECISION}`);
  }
}
...
function classifyDiffMode(files) {
  ...
  if (missing.length > 0) fail(`Phase 37-uiT PR diff missing required file(s): ${missing.join(', ')}`);
  return 'pr-diff';
}
...
function assertForbiddenChanges(files, mode) {
  ...
  for (const file of files) {
    if (!ALLOWED_FILES.has(file)) fail(`Changed file is outside Phase 37-uiT allowlist: ${file}`);
...
function assertWorkflow(workflow) {
  assertIncludes('workflow', workflow, [
    'Phase 37-uiS validator retained as historical reference; not run as Phase 37-uiT merge-blocking gate.',
    '# node scripts/validate-phase37-uis-dynamic-canvas-themes-design-gate.js',
    'Validate Phase 37-uiT Dynamic Canvas Themes Single-Surface Scope Gate',
    `node ${VALIDATOR_FILE}`
  ]);
  if (/git\s+fetch/.test(workflow)) fail('workflow must not include a shell git fetch step');
  if (workflow.includes('continue-on-error: true')) fail('workflow must not mark Phase 37-uiT validation continue-on-error');
  ...
  if (activeValidatorCommands.length !== 1 || activeValidatorCommands[0] !== `node ${VALIDATOR_FILE}`) {
    fail(`workflow must run only the Phase 37-uiT validator as the active Phase 37 validator. Found: ${activeValidatorCommands.join(', ')}`);
  }
}
...
function assertDocs(review, summary, seed) {
  ...
  assertIncludes('Phase 37-uiT tokens', combined, REQUIRED_TOKENS);
  assertDecisionToken(combined);
  assertIncludes('candidate surface inventory', review, CANDIDATE_SURFACES);
  assertIncludes('candidate surface comparison columns', review.toLowerCase(), COMPARISON_COLUMNS);
  assertIncludes('selected-surface constraints', combined, SELECTED_SURFACE_CONSTRAINTS);
  assertIncludes('candidate rows', review, CANDIDATE_ROWS);
  assertIncludes('Phase 37-uiU seed decision options', seed, SEED_DECISION_OPTIONS);
  assertIncludes('Phase 37-uiU seed required text', seed, SEED_REQUIRED_TEXT);
...
function assertNoGeneratedArtifacts(files) {
  for (const file of files) {
    if (/^(dist|coverage|test-results|playwright-report)\//.test(file)) {
      fail(`Generated artifact must not be part of Phase 37-uiT diff: ${file}`);
    }
  }
}
...
function main() {
  ...
  console.log(`Phase 37-uiT Dynamic Canvas Themes Single-Surface Scope Gate validator passed (${mode}).`);
}
