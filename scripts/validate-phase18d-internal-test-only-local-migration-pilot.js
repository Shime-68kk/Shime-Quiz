...
function packageGuard() {
  ...
  if (changed.has(`package.json`))      fail(`package.json must not change in Phase 18D`);
  if (changed.has(`package-lock.json`)) fail(`package-lock.json must not change in Phase 18D`);
}
...
function noSrcChangesGuard() {
  for (const file of changedFiles()) {
    ...
    if (firstSegment === 'src') fail(`src/ file changed in Phase 18D (forbidden): ${file}`);
  }
}
...
function scopeGuard() {
  for (const file of changedFiles()) {
    ...
    if (phase18dAllowedChangedFiles.has(file)) continue;
    if (file === `package.json` || file === `package-lock.json`) fail(`${file} must not change in Phase 18D`);
    const firstSegment = file.indexOf('/') >= 0 ? file.slice(0, file.indexOf('/')) : file;
    if (firstSegment === 'src') fail(`src/ file changed in Phase 18D (forbidden): ${file}`);
    if (firstSegment === 'e2e') fail(`e2e/ file changed in Phase 18D (forbidden): ${file}`);
