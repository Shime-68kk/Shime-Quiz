...
function scopeGuard(){ const changed=changedFiles(); for(const file of changed){ if(generatedArtifacts.some((a)=>file===a||file.startsWith(`${a}/`))) continue; if(file.startsWith('.claude/')) continue; if(forbiddenChangedFiles.includes(file) && !allowedChangedFiles.has(file)) fail(`Forbidden file changed: ${file}`); if(forbiddenChangedPrefixes.some((p)=>file.startsWith(p)) && !allowedChangedFiles.has(file)) fail(`Forbidden path changed: ${file}`); if(!allowedChangedFiles.has(file)) fail(`Unexpected changed file for Phase 12F scope: ${file}`); } }
