...
`scripts/validate-phase20b-real-user-testing-data-safety-feedback.js`,
  ...
  // engine audit docs/static-validator/CI files while preserving the
  // Phase 11C backup-transfer guardrails.
...
function assertNoUnsupportedPositiveClaims(file) {
  ...
  const patterns = [
    { re: /backup format (?:changed|updated|implemented)\??\s*(yes|pass)/i, label: 'backup format changed' },
    { re: /checksum implemented\??\s*(yes|pass)/i, label: 'checksum implemented' },
    { re: /compression implemented\??\s*(yes|pass)/i, label: 'compression implemented' },
    { re: /encryption implemented\??\s*(yes|pass)/i, label: 'encryption implemented' },
    { re: /import preview implemented\??\s*(yes|pass)/i, label: 'import preview implemented' },
    { re: /merge\/replace\/keep-both implemented\??\s*(yes|pass)/i, label: 'merge/replace/keep-both implemented' },
  ...
  const safeNegation = /\b(no|not|does not|do not|without|unless|was not|were not|is not|isn't|doesn't|future|plan|planned|requirements|documented|forbidden claims|do not claim)\b|không/i;
...
requireIncludes('docs/backup-transfer-safety-hardening.md', [
  'Phase 11C',
  'Backup Format / Transfer Safety Hardening',
  'completed/merged through Phase 11B',
  'current portability remains manual backup/export/import',
  'no backup format/runtime behavior changes are implemented by this phase',
...
requireAny('docs/backup-transfer-safety-hardening.md', ['checksum or error detection', 'checksum/error-detection'], 'checksum or error detection');
requireIncludes('README.md', ['docs/backup-transfer-safety-hardening.md', 'manual backup/export/import', 'No backup format changed']);
requireIncludes('RELEASE_QA_V2.md', ['Phase 11C', 'Backup Format / Transfer Safety Hardening', 'validate-backup-transfer-safety-hardening.js']);
requireIncludes('docs/cross-device-transfer-ux-decision.md', ['backup transfer safety hardening']);
requireIncludes('docs/cross-device-export-import.md', ['backup transfer safety hardening']);
requireIncludes('.github/workflows/e2e-smoke.yml', ['node scripts/validate-backup-transfer-safety-hardening.js']);

const pkg = JSON.parse(read('package.json'));
const lock = JSON.parse(read('package-lock.json'));
if (pkg.version !== '2.0.0-beta.1') fail(`package.json version changed: expected 2.0.0-beta.1, got ${pkg.version}`);
if (lock.version !== '2.0.0-beta.1') fail(`package-lock.json version changed: expected 2.0.0-beta.1, got ${lock.version}`);
const expectedDeps = {
  dependencies: {
...
try {
  ...
  for (const file of [...changed, ...untracked].filter((name) => !generatedPrefix.test(name) && !name.startsWith('.claude/'))) {
    if (!allowedChanged.has(file)) fail(`unexpected changed/untracked file outside Phase 11C scope: ${file}`);
    ...
    if (/^(src|app|components|lib)\//.test(file) && !allowedChanged.has(file)) fail(`runtime source file changed unexpectedly: ${file}`);
...
if (failures.length) {
  console.error('Backup transfer safety hardening validation failed:');
  for (const failure of failures) console.error(`- ${failure}`);
  process.exit(1);
}
console.log('Backup transfer safety hardening validation passed.');
