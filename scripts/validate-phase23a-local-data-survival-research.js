...
phase23aForwardCompatPaths.add(`docs/research/phase23e-data-survival-comprehension-evidence-run-plan.md`);
phase23aForwardCompatPaths.add(`docs/release/phase23e-data-survival-comprehension-plan-summary.md`);
phase23aForwardCompatPaths.add(`scripts/validate-phase23e-data-survival-comprehension-plan.js`);
...
phase23aForwardCompatPaths.add(`docs/release/phase23e-data-survival-comprehension-plan-summary.md`);
phase23aForwardCompatPaths.add(`scripts/validate-phase23e-data-survival-comprehension-plan.js`);
for (const path of phase23fForwardCompatPaths) phase23aForwardCompatPaths.add(path);
for (const path of phase24aForwardCompatPaths) phase23aForwardCompatPaths.add(path);
for (const path of phase24bForwardCompatPaths) phase23aForwardCompatPaths.add(path);
...
function validateChangedScope() {
  ...
  for (const file of files) {
    ...
    if (file.startsWith(`scripts/validate-`) && file.endsWith(`.js`) && file !== VALIDATOR && file !== `scripts/validate-phase24d-hf1-validator-forward-compat-maintenance.js`) continue;
...
function validateHistoricalForwardCompat() {
  ...
  for (const file of changedValidators) {
    ...
    for (const line of diff.split(/\r?\n/)) {
      ...
      if (![...phase23aForwardCompatPaths, ...phase23eForwardCompatPaths, ...phase23fForwardCompatPaths, ...phase24aForwardCompatPaths, ...phase24bForwardCompatPaths, ...phase24cForwardCompatPaths, ...phase24dForwardCompatPaths, ...phase24dHf1ForwardCompatPaths].some(path => line.includes(path))) {
        fail(`${file} has non-Phase-23A forward-compat addition: ${line}`);
      }
      for (const path of [...phase23aForwardCompatPaths, ...phase23eForwardCompatPaths, ...phase23fForwardCompatPaths, ...phase24aForwardCompatPaths, ...phase24bForwardCompatPaths, ...phase24cForwardCompatPaths, ...phase24dForwardCompatPaths, ...phase24dHf1ForwardCompatPaths]) {
