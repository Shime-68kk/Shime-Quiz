 ...
 *
 * Phase 14M static validator — FSRS Metadata Backup / Import / Export Hardening.
 * Modeled after validate-phase14l-production-enrollment-wiring.js.
 */

import fs from 'node:fs';
import { execSync } from 'node:child_process';

const DOCS_FILE = 'docs/phase14m-fsrs-metadata-backup-import-export-hardening.md';
const VALIDATOR_SCRIPT = 'scripts/validate-phase14m-fsrs-metadata-backup-import-export-hardening.js';
const WORKFLOW_FILE = '.github/workflows/e2e-smoke.yml';
const TEST_FILE = 'tests/unit/fsrsMetadataBackupImportExportHardening.test.js';
...
const phase14mAllowedChangedFiles = new Set([
  // Phase 14M new files
  DOCS_FILE,
  VALIDATOR_SCRIPT,
  TEST_FILE,
  WORKFLOW_FILE,
  // Phase 14K/14L validators updated for Phase 14M allowlist cascade
  PHASE14K_VALIDATOR,
  PHASE14L_VALIDATOR,
...
phase14mAllowedChangedFiles.add(`docs/research/phase23e-data-survival-comprehension-evidence-run-plan.md`);
phase14mAllowedChangedFiles.add(`docs/release/phase23e-data-survival-comprehension-plan-summary.md`);
phase14mAllowedChangedFiles.add(`scripts/validate-phase23e-data-survival-comprehension-plan.js`);
phase14mAllowedChangedFiles.add(`docs/release/phase23f-phase23-decision-gate.md`);
phase14mAllowedChangedFiles.add(`docs/research/phase23f-data-survival-decision-matrix.md`);
phase14mAllowedChangedFiles.add(`scripts/validate-phase23f-phase23-decision-gate.js`);
phase14mAllowedChangedFiles.add(`docs/research/phase24a-residual-direct-storage-audit.md`);
phase14mAllowedChangedFiles.add(`docs/release/phase24a-residual-direct-storage-audit-summary.md`);
phase14mAllowedChangedFiles.add(`scripts/validate-phase24a-residual-direct-storage-audit.js`);
phase14mAllowedChangedFiles.add(`docs/research/phase24b-storage-adapter-coverage-boundary-decision.md`);
phase14mAllowedChangedFiles.add(`docs/release/phase24b-storage-adapter-boundary-summary.md`);
phase14mAllowedChangedFiles.add(`scripts/validate-phase24b-storage-adapter-boundary-decision.js`);
// Phase 24C forward-compat entries (Help Tour StorageAdapter scaffold)
phase14mAllowedChangedFiles.add(`src/ui/helpTourStorage.js`);
phase14mAllowedChangedFiles.add(`src/ui/helpTour.js`);
phase14mAllowedChangedFiles.add(`tests/unit/helpTourStorageAdapterScaffold.test.js`);
phase14mAllowedChangedFiles.add(`docs/research/phase24c-help-tour-storage-adapter-scaffold.md`);
phase14mAllowedChangedFiles.add(`docs/release/phase24c-help-tour-storage-adapter-scaffold-summary.md`);
phase14mAllowedChangedFiles.add(`scripts/validate-phase24c-help-tour-storage-adapter-scaffold.js`);
// Phase 24D forward-compat entries (Backup/Export/Restore Adapter-Awareness Design)
phase14mAllowedChangedFiles.add(`docs/research/phase24d-backup-export-restore-adapter-awareness-design.md`);
phase14mAllowedChangedFiles.add(`docs/release/phase24d-backup-export-restore-adapter-awareness-summary.md`);
phase14mAllowedChangedFiles.add(`scripts/validate-phase24d-backup-export-restore-adapter-awareness-design.js`);
phase14mAllowedChangedFiles.add(`docs/research/phase23b-data-survival-ux-copy-decision.md`);
phase14mAllowedChangedFiles.add(`docs/release/phase23b-data-survival-ux-copy-summary.md`);
phase14mAllowedChangedFiles.add(`scripts/validate-phase23b-data-survival-ux-copy.js`);
phase14mAllowedChangedFiles.add(`docs/research/phase23c-backup-health-last-backup-indicator-design.md`);
phase14mAllowedChangedFiles.add(`docs/release/phase23c-backup-health-design-summary.md`);
phase14mAllowedChangedFiles.add(`scripts/validate-phase23c-backup-health-design.js`);
phase14mAllowedChangedFiles.add(`docs/research/phase23d-backup-reminder-risk-friction-design.md`);
phase14mAllowedChangedFiles.add(`docs/release/phase23d-backup-reminder-risk-friction-summary.md`);
phase14mAllowedChangedFiles.add(`scripts/validate-phase23d-backup-reminder-risk-friction-design.js`);
phase14mAllowedChangedFiles.add(`docs/research/phase24d-hf1-validator-forward-compat-maintenance.md`);
phase14mAllowedChangedFiles.add(`docs/release/phase24d-hf1-validator-forward-compat-summary.md`);
phase14mAllowedChangedFiles.add(`scripts/register-phase-forward-compat.js`);
phase14mAllowedChangedFiles.add(`scripts/validate-phase24d-hf1-validator-forward-compat-maintenance.js`);
...
function fail(message) {
  console.error(`Phase 14M FSRS metadata backup/import/export hardening validation failed: ${message}`);
  process.exit(1);
}

function warn(message) {
  console.warn(`Phase 14M FSRS metadata backup/import/export hardening validation warning: ${message}`);
}
...
function scopeGuard() {
  ...
  // Phase 20D naming-cleanup compat: line neutralized for exact 2.0.0-beta.1 transition.
    if (file.startsWith('e2e/')) fail(`E2E file changed in Phase 14M: ${file}`);
    if (file === STUDY_ROOM) fail(`StudyRoom.jsx must not change in Phase 14M`);
    if (file === DASHBOARD) fail(`Dashboard.jsx must not change in Phase 14M`);
    if (file === WRAPPER_SOURCE) fail(`fsrsWrapper.js must not change in Phase 14M`);
    fail(`Unexpected changed file for Phase 14M scope: ${file}`);
  }
}
...
function backupRestoreSourceGuard() {
  const backupSource = read(V2_BACKUP_RESTORE);

  // Must not import enrollment helpers
  if (backupSource.includes('isFsrsNewCardEnrollmentEligible')) {
    fail(`${V2_BACKUP_RESTORE} must not call isFsrsNewCardEnrollmentEligible — restore must not enroll cards`);
  }
  if (backupSource.includes('scheduleDormantFsrsReview')) {
    fail(`${V2_BACKUP_RESTORE} must not call scheduleDormantFsrsReview — restore must not enroll cards`);
  }
  // No active FSRS scheduling
  if (/\.next\s*\(/.test(backupSource)) {
    fail(`${V2_BACKUP_RESTORE} must not call .next() — active FSRS scheduling is disabled in Phase 14M`);
  }
  // No onMount/boot-time enrollment
  if (/onMount.*enroll/i.test(backupSource)) {
    fail(`${V2_BACKUP_RESTORE} must not contain onMount enrollment`);
  }
  if (/app.*boot.*enroll/i.test(backupSource)) {
    fail(`${V2_BACKUP_RESTORE} must not contain app-boot enrollment`);
  }
  if (/session.*start.*enroll/i.test(backupSource)) {
    fail(`${V2_BACKUP_RESTORE} must not contain session-start enrollment`);
  }
  // No Two-Step UI reference
  if (/FsrsTwoStepScaffold/.test(backupSource)) {
    fail(`${V2_BACKUP_RESTORE} must not reference FsrsTwoStepScaffold`);
  }
}
...
function storageSourceGuard() {
  ...
  if (!storageSource.includes('schedulerKind')) {
    fail(`${STORAGE_SOURCE} must handle schedulerKind in normalization`);
  }
  if (!storageSource.includes('schedulerVersion')) {
    fail(`${STORAGE_SOURCE} must handle schedulerVersion in normalization`);
  }
  if (!storageSource.includes('fsrsPayload')) {
    fail(`${STORAGE_SOURCE} must handle fsrsPayload in normalization`);
  }
  if (!storageSource.includes('fsrsReviewLogs')) {
    fail(`${STORAGE_SOURCE} must handle fsrsReviewLogs in normalization`);
  }
  ...
  if (/\.next\s*\(/.test(storageSource)) {
    fail(`${STORAGE_SOURCE} must not call .next() — active FSRS scheduling is disabled in Phase 14M`);
  }
}
...
function adapterGuard() {
  ...
  if (/\.next\s*\(/.test(adapterSource)) {
    fail(`${ADAPTER_SOURCE} must not call .next() — production FSRS scheduling is disabled in Phase 14M`);
  }
...
function testCoverageGuard() {
  ...
  // Must test the cap at 20
  if (!testSource.includes('FSRS_REVIEW_LOG_CAP')) {
    fail(`${TEST_FILE} must reference FSRS_REVIEW_LOG_CAP`);
  }
  // Must test legacy compatibility
  if (!testSource.includes('SM2_LEGACY_RECORD') && !testSource.toLowerCase().includes('sm-2')) {
    fail(`${TEST_FILE} must test legacy SM-2 records`);
  }
  // Must test invalid metadata
  if (!testSource.toLowerCase().includes('malform')) {
    fail(`${TEST_FILE} must test malformed FSRS metadata handling`);
  }
}
...
function validate() {
  ...
  testCoverageGuard();
  docsGuard();
  console.log('Phase 14M FSRS metadata backup/import/export hardening validation passed.');
}
