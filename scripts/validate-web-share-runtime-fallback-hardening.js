...
try {
  ...
  for (const file of [...changed, ...untracked].filter((name) => !generatedPrefix.test(name))) {
    ...
    if (/^src\//.test(file) && file !== 'src/components/learning/V2BackupRestorePanel.jsx') fail(`unrelated runtime source file changed unexpectedly: ${file}`);
    if (file === 'package.json' || file === 'package-lock.json') fail(`package/dependency file changed unexpectedly: ${file}`);
    if (file !== 'src/components/learning/V2BackupRestorePanel.jsx' && /(storage|schema|parser|importValidator|v2BackupRestore|localStorageSync)/i.test(file)) fail(`storage/import/backup behavior source changed unexpectedly: ${file}`);
  }
} catch {
