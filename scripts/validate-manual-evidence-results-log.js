...
function requireIncludes(file, phrases) {
  ...
  for (const phrase of phrases) {
    if (!normalized.includes(norm(phrase))) {
...
function requireLinkOrMention(file) {
  const source = norm(read(file));
  if (!source.includes('manual-evidence-results-log.md') && !source.includes('manual evidence results log')) {
    fail(`${file} must link to or mention the manual evidence results log`);
  }
}
...
function assertNoForbiddenPositiveClaims(file) {
  ...
  const explicitPositiveClaims = [
    ...
    { re: /release package created\/published\??\s*(yes|pass)/i, label: 'release package created/published' },
    ...
    { re: /edugen bundled\??\s*(yes|pass)/i, label: 'EduGen bundled' },
    { re: /frontend-only pdf\/docx\/pptx\/zip conversion\??\s*(yes|pass)/i, label: 'frontend-only document conversion' },
  ];
  for (const item of explicitPositiveClaims) {
    if (item.re.test(source)) {
      fail(`${file} appears to make an unsupported positive claim: ${item.label}`);
    }
  }
}

requireIncludes('docs/manual-evidence-results-log.md', [
  'Phase 10S',
  'Manual Evidence Run Log',
...
requireAny('docs/manual-evidence-results-log.md', [
  'Release package has not been created or published',
  'Release package has not been created.\n- Release package has not been published',
], 'release package has not been created or published');

requireLinkOrMention('README.md');
requireIncludes('RELEASE_QA_V2.md', ['Phase 10S', 'manual evidence results log', 'validate-manual-evidence-results-log.js']);
...
requireIncludes('.github/workflows/e2e-smoke.yml', ['node scripts/validate-manual-evidence-results-log.js']);

const pkg = JSON.parse(read('package.json'));
const lock = JSON.parse(read('package-lock.json'));
if (pkg.version !== '2.0.0-beta-ai.1') {
  fail(`package.json version changed: expected 2.0.0-beta-ai.1, got ${pkg.version}`);
}
if (lock.version !== '2.0.0-beta-ai.1') {
  fail(`package-lock.json version changed: expected 2.0.0-beta-ai.1, got ${lock.version}`);
}
const expectedDeps = {
  dependencies: {
...
for (const field of ['dependencies', 'devDependencies']) {
  ...
  if (actual !== expected) {
    fail(`package.json ${field} changed unexpectedly`);
  }
}
...
if (lockRoot) {
  for (const field of ['dependencies', 'devDependencies']) {
    ...
    const expected = JSON.stringify(expectedDeps[field], Object.keys(expectedDeps[field]).sort());
    if (actual !== expected) {
      fail(`package-lock.json root ${field} changed unexpectedly`);
    }
  }
}
...
for (const rel of walk(root)) {
  ...
  if (secretPatterns.some((pattern) => pattern.test(base) || pattern.test(rel))) {
    fail(`secret-like/local-only file must not be included: ${rel}`);
  }
}
...
if (failures.length) {
  console.error('Manual evidence results log validation failed:');
  for (const failure of failures) console.error(`- ${failure}`);
  process.exit(1);
}

console.log('Manual evidence results log validation passed.');
