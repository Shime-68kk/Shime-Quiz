 ...
 *
 * Phase 20A static validator — Beta Local-First Hybrid Stabilization Gate.
 *
 * Phase 20A is docs/static-validator/CI-only. It does not implement sync,
 * account/auth/backend, storage changes, FSRS changes, backup/export/restore
 * changes, UI, tests, or package changes. Its only deliverables are this
 * validator, the Phase 20A ADR, and CI registration after Phase 19D.
 */

import fs from 'node:fs';
import { execSync } from 'node:child_process';

const ADR_FILE           = `docs/adr/phase20a-beta-local-first-hybrid-stabilization.md`;
const VALIDATOR_SCRIPT   = `scripts/validate-phase20a-beta-local-first-hybrid-stabilization.js`;
const WORKFLOW_FILE      = `.github/workflows/e2e-smoke.yml`;
const PHASE19D_VALIDATOR = `scripts/validate-phase19d-no-cloud-default-off-trust-copy.js`;

const phase20aCoreFiles = [
  ADR_FILE,
  VALIDATOR_SCRIPT,
];

// Phase 20A forward-compat entries: the only paths historical validators may add.
const phase20aForwardCompatEntries = [
...
// forward-compat lists from prior phases. They are NOT additions in Phase 20A;
...
const phase20aAllowedChangedFiles = new Set([
  WORKFLOW_FILE,
  PHASE19D_VALIDATOR,
  ...phase20aCoreFiles,
  ...prePhase20aBaselineFiles,
...
phase20aAllowedChangedFiles.add(`docs/research/phase23e-data-survival-comprehension-evidence-run-plan.md`);
phase20aAllowedChangedFiles.add(`docs/release/phase23e-data-survival-comprehension-plan-summary.md`);
phase20aAllowedChangedFiles.add(`scripts/validate-phase23e-data-survival-comprehension-plan.js`);
phase20aAllowedChangedFiles.add(`docs/release/phase23f-phase23-decision-gate.md`);
phase20aAllowedChangedFiles.add(`docs/research/phase23f-data-survival-decision-matrix.md`);
phase20aAllowedChangedFiles.add(`scripts/validate-phase23f-phase23-decision-gate.js`);
phase20aAllowedChangedFiles.add(`docs/research/phase24a-residual-direct-storage-audit.md`);
phase20aAllowedChangedFiles.add(`docs/release/phase24a-residual-direct-storage-audit-summary.md`);
phase20aAllowedChangedFiles.add(`scripts/validate-phase24a-residual-direct-storage-audit.js`);
phase20aAllowedChangedFiles.add(`docs/research/phase24b-storage-adapter-coverage-boundary-decision.md`);
phase20aAllowedChangedFiles.add(`docs/release/phase24b-storage-adapter-boundary-summary.md`);
phase20aAllowedChangedFiles.add(`scripts/validate-phase24b-storage-adapter-boundary-decision.js`);
// Phase 24C forward-compat entries (Help Tour StorageAdapter scaffold)
phase20aAllowedChangedFiles.add(`src/ui/helpTourStorage.js`);
phase20aAllowedChangedFiles.add(`src/ui/helpTour.js`);
phase20aAllowedChangedFiles.add(`tests/unit/helpTourStorageAdapterScaffold.test.js`);
phase20aAllowedChangedFiles.add(`docs/research/phase24c-help-tour-storage-adapter-scaffold.md`);
phase20aAllowedChangedFiles.add(`docs/release/phase24c-help-tour-storage-adapter-scaffold-summary.md`);
phase20aAllowedChangedFiles.add(`scripts/validate-phase24c-help-tour-storage-adapter-scaffold.js`);
// Phase 24D forward-compat entries (Backup/Export/Restore Adapter-Awareness Design)
phase20aAllowedChangedFiles.add(`docs/research/phase24d-backup-export-restore-adapter-awareness-design.md`);
phase20aAllowedChangedFiles.add(`docs/release/phase24d-backup-export-restore-adapter-awareness-summary.md`);
phase20aAllowedChangedFiles.add(`scripts/validate-phase24d-backup-export-restore-adapter-awareness-design.js`);
phase20aAllowedChangedFiles.add(`docs/research/phase23b-data-survival-ux-copy-decision.md`);
phase20aAllowedChangedFiles.add(`docs/release/phase23b-data-survival-ux-copy-summary.md`);
phase20aAllowedChangedFiles.add(`scripts/validate-phase23b-data-survival-ux-copy.js`);
phase20aAllowedChangedFiles.add(`docs/research/phase23c-backup-health-last-backup-indicator-design.md`);
phase20aAllowedChangedFiles.add(`docs/release/phase23c-backup-health-design-summary.md`);
phase20aAllowedChangedFiles.add(`scripts/validate-phase23c-backup-health-design.js`);
phase20aAllowedChangedFiles.add(`docs/research/phase23d-backup-reminder-risk-friction-design.md`);
phase20aAllowedChangedFiles.add(`docs/release/phase23d-backup-reminder-risk-friction-summary.md`);
phase20aAllowedChangedFiles.add(`scripts/validate-phase23d-backup-reminder-risk-friction-design.js`);
phase20aAllowedChangedFiles.add(`docs/research/phase24d-hf1-validator-forward-compat-maintenance.md`);
phase20aAllowedChangedFiles.add(`docs/release/phase24d-hf1-validator-forward-compat-summary.md`);
phase20aAllowedChangedFiles.add(`scripts/register-phase-forward-compat.js`);
phase20aAllowedChangedFiles.add(`scripts/validate-phase24d-hf1-validator-forward-compat-maintenance.js`);
...
const generatedArtifacts = [
  ...
  `.env.local`,
  `.git`,
  `phase20a-beta-local-first-hybrid-stabilization.patch`,
  `phase20a-beta-local-first-hybrid-stabilization.zip`,
  `phase20a-beta-local-first-hybrid-stabilization-handoff.md`,
];
...
// Required Phase 17–18 evidence terms
const requiredPhase17EvidenceTerms = [
  `phase 17a`,
  `phase 17b`,
  `phase 17c`,
  `phase 17d`,
  ...
  `phase 17h`,
  `phase 17i`,
  `backup/rollback harness before migration`,
  `storageadapter`,
  `test-only`,
  `migration journal`,
];

const requiredPhase18EvidenceTerms = [
  `phase 18a`,
  `phase 18b`,
  `phase 18c`,
  `phase 18d`,
...
const requiredPhase19EvidenceTerms = [
  `phase 19a`,
  `phase 19b`,
  `phase 19c`,
  `phase 19d`,
...
const requiredDecisionTerms = [
  ...
  `phase 20a is a stabilization audit gate only`,
  `phase 20a does not implement sync runtime`,
  `phase 20a does not implement account/cloud/auth/backend`,
  `phase 20a does not implement storage backend switch`,
  ...
  `conflict model remains a design decision only`,
  `no-cloud/default-off trust copy remains the user-trust boundary`,
  `phase 20b should handle real user testing`,
  `phase 20c should handle performance/quota/import stress test plan`,
  `phase 20d should decide local-first hybrid beta-ready or hold`,
];

// Required safety invariant terms
const requiredSafetyInvariantTerms = [
  ...
  `backup-before-merge remains required`,
  ...
  `no sync runtime before manual transfer`,
...
// Required Phase 20B/20C/20D alignment terms
const requiredPhase20AlignmentTerms = [
  `phase 20b`,
  `phase 20c`,
  `phase 20d`,
  `real user testing`,
...
function fail(message) {
  console.error(`Phase 20A validation failed: ${message}`);
  process.exit(1);
}

function warn(message) {
  console.warn(`Phase 20A validation warning: ${message}`);
}
...
function requiredFilesGuard() {
  for (const file of [ADR_FILE, VALIDATOR_SCRIPT, WORKFLOW_FILE, PHASE19D_VALIDATOR]) {
    read(file);
  }
}
...
function workflowGuard() {
  ...
  if (text.indexOf(phase20aStr) <= text.indexOf(phase19dStr)) {
    fail(`${WORKFLOW_FILE} must register Phase 20A validator after Phase 19D`);
  }
...
function packageGuard() {
  ...
  if (!allowed.has(pkgVer)) fail('package.json version unexpected in Phase 20A: ' + pkgVer);
  if (!allowed.has(lockVer)) fail('package-lock.json version unexpected in Phase 20A: ' + lockVer);
}
...
function noSrcTestsE2eChangesGuard() {
  ...
  `scripts/validate-phase22f-actual-stress-run.js`,].includes(file)) continue;
    if (phase20aAllowedChangedFiles.has(file)) continue;
    if (file === 'src/version.js') { /* Phase 20D naming-cleanup compat */ } else if (firstSegment(file) === 'src') fail(`src/ file changed in Phase 20A (forbidden): ${file}`);
    if (firstSegment(file) === 'tests') fail(`tests/ file changed in Phase 20A (forbidden): ${file}`);
    if (firstSegment(file) === 'e2e') fail(`e2e/ file changed in Phase 20A (forbidden): ${file}`);
  }
}
...
function runtimeGuard(label, files) {
  ...
  for (const file of files) {
    if (changed.has(file)) fail(`${label} changed in Phase 20A (forbidden): ${file}`);
  }
}
...
function scopeGuard() {
  ...
  // Phase 20D naming-cleanup compat: line neutralized for exact 2.0.0-beta.1 transition.
    ...
    if (firstSegment(file) === 'e2e') fail(`e2e/ file changed in Phase 20A (forbidden): ${file}`);
    if (firstSegment(file) === 'tests') fail(`tests/ file changed in Phase 20A (forbidden): ${file}`);
    if (firstSegment(file) === 'docs' && !phase20aAllowedChangedFiles.has(file)) {
      fail(`Unexpected docs/ file changed in Phase 20A: ${file}`);
    }
    if (file.startsWith(`scripts/validate-`) && file.endsWith(`.js`)) continue;
    warn(`Unexpected file outside allowed Phase 20A scope (non-fatal): ${file}`);
  }
}

function forbiddenRuntimeFilesGuard() {
  for (const file of forbiddenRuntimeFiles) {
    if (fs.existsSync(file)) fail(`Phase 20A must not introduce forbidden runtime file: ${file}`);
  }
}
...
function isForbiddenClaimSection(line) {
  return /^##\s+(What is not stable enough for beta|Forbidden claims|Claims.*must not|claims shime must not)/i.test(normalize(line));
}
  ...
  for (const validatorFile of changedValidators) {
    ...
    for (const line of addedLines) {
      ...
      for (const path of extractedPaths) {
        ...
        if (phase20aForwardCompatEntries.includes(path)) continue;
        ...
        if (path.startsWith(`docs/`) || path.startsWith(`tests/`) || path.startsWith(`scripts/`)) {
          fail(`Historical validator ${validatorFile} adds non-Phase-20A path entry: '${path}'`);
        }
      }
    }
  }
...
function validate() {
  ...
  requireTerms(ADR_FILE, requiredPhase17EvidenceTerms);
  requireTerms(ADR_FILE, requiredPhase18EvidenceTerms);
  requireTerms(ADR_FILE, requiredPhase19EvidenceTerms);
  requireTerms(ADR_FILE, requiredSafetyInvariantTerms);
  requireTerms(ADR_FILE, requiredPhase20AlignmentTerms);
  forbiddenPositiveClaimGuardForFile(ADR_FILE);
  generatedArtifactGuard();
  historicalValidatorForwardCompatGuard();
  console.log(`Phase 20A Beta Local-First Hybrid Stabilization validation passed.`);
}
